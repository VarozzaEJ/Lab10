...
public class ListTester {
	...
	private void runTests() {
		...
		//3-element to 1-element
			// Scenario: 51
		testSingleElementList(ABC_iteratorNextRemove_NextRemove_C, "ABC_iteratorNextRemove_NextRemove_C", LIST_C, STRING_C);
			// Scenario: 52
	 ...
	 */
	private IndexedUnsortedList<Integer> ABC_iteratorNextRemove_NextRemove_C() {
		IndexedUnsortedList<Integer> list = AB_addAfterBC_ABC();
		Iterator<Integer> iter = list.iterator();
		iter.next();
		iter.remove();
	...
	private Scenario<Integer> ABC_iteratorNextRemove_NextRemove_C = () -> ABC_iteratorNextRemove_NextRemove_C();
	...
	private void testThreeElementList(Scenario<Integer> scenario, String scenarioName, Integer[] contents, String contentsString) {
		...
		try {
			...
			printTest(scenarioName + "_iterNext_testIterNext", testIterNext(WrapIt.prep(scenario.build()).next().next().getIterator(), contents[2], Result.MatchingValue));
			printTest(scenarioName + "_iterNext_testIterRemove", testIterRemove(WrapIt.prep(scenario.build()).next().next().getIterator(), Result.NoException));
